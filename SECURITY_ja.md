...
脆弱性の報告は、重大度にかかわらず **GitHub Private Security Advisory** に一本化しています。

1. セキュリティ脆弱性については、重大度にかかわらず公開の GitHub Issue を作成**しないでください**
2. 以下の URL からプライベートセキュリティアドバイザリを作成してください：
   https://github.com/elvezjp/excel2md/security/advisories/new

脆弱性ではないセキュリティ関連のご質問（ベストプラクティス、設定方法など）は [ご質問](#ご質問) セクションをご確認ください。
...
- マクロ（`.xlsm` ファイル）— 読み込みのみ。VBA はロード時に破棄され、**決して実行されません**（下記の注記参照）
- 外部リンクと参照
- 埋め込みオブジェクト
- 副作用を持つ可能性のある数式
...
`.xlsm` は `openpyxl.load_workbook(..., keep_vba=False)`（既定値）で読み込まれ、VBA バイナリはロード時点で破棄されます。そのため、`Auto_Open` や `Workbook_Open` などの自動実行マクロが excel2md によって発火することはなく、マクロのコードが Markdown 出力に書き出されることもありません。
...
これらの依存関係のセキュリティアドバイザリを監視し、必要に応じて更新しています。Dependabot アラートは依存更新／PR 作成で修正対応します。依存パッケージのバージョン更新のみで対応できる場合、本体バージョンは変更しません。
