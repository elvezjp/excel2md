...
def run(input_path: str, output_path: Optional[str], args):
    ...
    # DrawingML 索引 (Issue #26)
    # 画像抽出と shapes モードの Mermaid 図形抽出で共用し、xlsx ZIP のオープンと
    # workbook.xml / rels の解析をワークブックにつき1回に抑える。
    needs_drawing_index = (
        (opts.get("mermaid_enabled", False) and opts.get("mermaid_detect_mode") == "shapes")
        or (opts.get("csv_markdown_enabled", True) and opts.get("image_extraction", True))
    )
    drawing_index = WorkbookDrawingIndex(input_path) if needs_drawing_index else None
    ...
    try:
        ...
        for sname in sheets:
            ...
            if emit_normal_md and opts.get("mermaid_enabled", False) and opts.get("mermaid_detect_mode") == "shapes":
                shapes_mermaid = _v14_extract_shapes_to_mermaid(input_path, ws, opts, drawing_index=drawing_index)
            ...
            if opts.get("csv_markdown_enabled", True):
                ...
                if opts.get("image_extraction", True):
                    cell_to_image = extract_images_from_sheet(ws, Path(csv_output_dir), sname, csv_basename, opts, xlsx_path=input_path, drawing_index=drawing_index)
