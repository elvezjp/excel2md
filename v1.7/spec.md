...
本書は、v1.6の仕様に**CSVマークダウンモードの拡張機能**（Mermaid出力対応、説明文除外オプション）を追加した仕様です。
...
## 13. リポジトリドキュメント（README.md）

リポジトリルートの `README.md` は、ユーザーがツールの概要・使い方・仕様を理解するためのドキュメントです。以下の内容を記載します。

### 13.1 ドキュメント構成

README.mdは以下のセクションで構成されます：
...
**タイトル**:
- ツール名と現在の仕様バージョンを明記（例: `Excel → Markdown Converter (Spec v1.5)`）

**概要**:
- ツールの目的を1〜2段落で説明
  - Excelブック（.xlsx/.xlsm）を読み取り、各シートの印刷領域を解析してMarkdown形式で自動生成するツールであること
  - v1.5では、Mermaidフローチャート変換とCSVマークダウン出力機能を追加したこと
- 簡潔で分かりやすい表現を心がける
...
**デフォルト動作（v1.5）**:
- Markdown出力に加えて、CSVマークダウンファイルを自動出力
- Mermaidフローチャート変換は無効（`--mermaid-enabled` で有効化）
...
1. **基本オプション（v1.3から継承）**:
   ...
   - `--hyperlink-mode`: ハイパーリンクの出力方法（`inline` / `inline_plain` / `footnote` / `both` / `text_only`）
     - CSVマークダウンでは `footnote`/`both` は `inline_plain` にフォールバック
   - `--align-detection`: 数値列の右寄せ判定
   - `--max-cells-per-table`: テーブル1つあたりの最大セル数
   - `--footnote-scope`: 脚注番号の採番方式
   - `--markdown-escape-level`: Markdown記号のエスケープレベル
   - その他、主要なオプションを列挙

2. **Mermaid関連オプション（v1.4で追加）**:
   - `--mermaid-enabled`: Mermaidフローチャート変換を有効化
   - `--mermaid-detect-mode`: 検出モード（column_headers / heuristic / shapes）
   - `--mermaid-direction`: フローチャートの方向（TD / LR / BT / RL）
   - `--mermaid-keep-source-table`: 元のテーブルも出力するか

3. **CSVマークダウン出力オプション（v1.5で追加、v1.7で拡張）**:
   - `--csv-output-dir`: CSVマークダウンファイルの出力先ディレクトリ
   - `--csv-include-metadata` / `--no-csv-include-metadata`: 検証用メタデータを含めるか（デフォルト: true）
   - `--csv-include-description` / `--no-csv-include-description`: 概要セクション（説明文）を含めるか（デフォルト: true）【v1.7で追加】
   - Mermaid出力は `--mermaid-enabled --mermaid-detect-mode shapes` で制御【v1.7で追加】
   - その他、CSV出力に関連するオプション

各オプションについて、オプション名、デフォルト値、説明を表形式で記載。

**使用例**:

複数の使用例をコードブロックとコメント付きで記載：

```bash
# 基本的な使い方（CSVマークダウン自動出力）
python v1.7/excel_to_md.py input.xlsx -o output.md

# CSVマークダウンにMermaidを含める（v1.7新機能）
python v1.7/excel_to_md.py input.xlsx --mermaid-enabled --mermaid-detect-mode shapes

# 説明文とメタデータを省略してトークン数を削減（v1.7新機能）
python v1.7/excel_to_md.py input.xlsx --no-csv-include-description --no-csv-include-metadata
...
- **CSVマークダウン出力機能**（§3.2.2）:
  ...
  - ファイル冒頭に概要セクション（ファイル情報、CSV生成方法、CSV形式の仕様など）を自動生成
  - ファイル末尾に検証用メタデータセクション（マークダウン形式）を自動生成
...
**Markdown出力例**:
- 実際の変換結果に近いサンプルをコードブロックで記載
- ファイルヘッダー（ファイル名、仕様バージョン、シート数、シート一覧）を含める
- シートセクションとテーブルの例を含める
- ハイパーリンクや脚注の例を含める

**CSVマークダウン出力例**:
- 実際の出力形式に基づいたサンプルをコードブロックで記載
- 概要セクション（ファイル情報、このファイルについて、CSV生成方法、CSV形式の仕様、検証用メタデータについて）を含める
- 複数シートのCSVコードブロック例を含める
- 検証用メタデータセクション（生成日時、元Excelファイル、各シート情報、全体の検証結果）を含める
...
#### 13.2.7 ライセンスとバージョン管理

- **最新仕様バージョン**: v1.7
  - v1.7: CSVマークダウンモード拡張（Mermaid出力対応、説明文・メタデータ除外オプション）
  - v1.6: ハイパーリンク平文出力モード（inline_plain）、シート分割出力機能（--split-by-sheet）
...
- **ライセンス**: MIT（予定）またはプロジェクトのライセンスを明記

#### 13.2.8 開発メモ（推奨）

- 各バージョンでコード内に `VERSION = "1.X"` を明記し、仕様との同期を保証
- v1.7では、v1.6の全機能 + CSVマークダウンモードの拡張機能を統合
- v1.5では、v1.3の全機能 + v1.4のMermaid機能 + CSVマークダウン出力機能を統合
- バージョン間の互換性を維持し、旧バージョンも引き続き使用可能

#### 13.2.9 リリースノート（推奨）

最新版（v1.7）のリリース情報を詳細に記載：

- **リリース日時**: 2025-12-25
- **主な新機能**:
  - CSVマークダウンモードの拡張機能
    - `--csv-include-description` / `--no-csv-include-description`: 概要セクション（説明文）の出力を制御（デフォルト: ON）
    - CSVマークダウンでも `--mermaid-enabled` が有効に（`mermaid_detect_mode="shapes"` の場合のみ）
  - 複数ファイルを変換・結合する際のトークン数削減に対応
- **破壊的変更**: なし（v1.6との後方互換性を維持）
- **使用例**:
  ```bash
  # Mermaid出力を含めてCSVマークダウンを生成
  python v1.7/excel_to_md.py input.xlsx --mermaid-enabled --mermaid-detect-mode shapes

  # 説明文とメタデータを省略してトークン数を削減
  python v1.7/excel_to_md.py input.xlsx --no-csv-include-description --no-csv-include-metadata
  ```
- **注意事項**: CSVマークダウンでのMermaid出力は `mermaid_detect_mode="shapes"` の場合のみ対応。`column_headers` や `heuristic` モードはCSVマークダウンではスキップされる（テーブル分割を行わないため）。

過去のリリース履歴（v1.6, v1.5, v1.4, v1.3等）も同様に記載。

### 13.3 更新方針

- 仕様変更時は必ずREADME.mdも同期して更新する
- 最新バージョンの仕様を正確に反映する
- 旧バージョンの情報は「基本情報」と「ライセンスとバージョン管理」セクションに簡潔に記載
- 詳細な仕様は仕様書（`v1.7/spec.md`）を参照する旨を明記する
...
### 目的
複数のExcelファイルを順次処理し、日付＋通番のディレクトリに出力するバッチ処理の動作確認を行う。

### バージョン別の試験対象
- **v1.3**:
  - `実験1/` ディレクトリ内の `.xlsx` / `.xlsm` ファイル
  - `実験2/` ディレクトリ内の `.xlsx` / `.xlsm` ファイル
- **v1.4**:
  - `実験1/` ディレクトリ内の `.xlsx` / `.xlsm` ファイル
  - `実験2/` ディレクトリ内の `.xlsx` / `.xlsm` ファイル
  - `実験3/` ディレクトリ内の `.xlsx` / `.xlsm` ファイル（フローチャート用、シェイプ検出モード適用）
  - `実験4/` ディレクトリ内の `.xlsx` / `.xlsm` ファイル（シンプルなExcel用、シェイプ検出モード適用）
- **v1.5**:
  - `実験1/` ディレクトリ内の `.xlsx` / `.xlsm` ファイル
  - `実験2/` ディレクトリ内の `.xlsx` / `.xlsm` ファイル
  - `実験3/` ディレクトリ内の `.xlsx` / `.xlsm` ファイル（フローチャート用、シェイプ検出モード適用）
...
### 処理仕様
1. **ディレクトリ名の生成**
   - 形式：`YYYYMMDDNNN`（例：`20251110001`）
   - `YYYYMMDD`：実行日付（8桁）
   - `NNN`：通番（3桁、001から開始、1日ごとにリセット）

2. **処理順序**
   - `実験1/` ディレクトリ内のファイルを先に処理
   - 次に `実験2/` ディレクトリ内のファイルを処理
   - v1.4またはv1.5の場合、次に `実験3/` ディレクトリ内のファイルを処理
   - v1.4またはv1.5の場合、最後に `実験4/` ディレクトリ内のファイルを処理
   - 各ディレクトリ内では、ファイル名の辞書順で処理

3. **出力先**
   - 生成されたディレクトリ名（例：`20251110001/`）を作成
   - 各Excelファイルの変換結果を、元のファイル名（拡張子を `.md` に変更）で保存
   - 例：`実験1/sample.xlsx` → `20251110001/実験1/sample.md`

4. **エラーハンドリング**
   - 処理失敗したファイルはエラーログに記録し、次のファイルへ継続
   - バッチ処理全体の成功/失敗を最後に報告

5. **ログ出力**
   - 処理開始時刻
   - 使用バージョン（v1.3、v1.4、またはv1.5）
   - 各ファイルの処理状況（成功/失敗）
   - 処理終了時刻
   - 処理統計（総ファイル数、成功数、失敗数）
   - **Mermaid統計（v1.4以降、Mermaid有効時）**
     - `mermaid_generated`: 生成に成功したテーブル数
     - `mermaid_skipped`: 判定不成立/生成失敗によりスキップしたテーブル数
     - `mermaid_fallbacks`: Mermaidから既存形式へフォールバックしたテーブル数
   - **CSVマークダウン出力統計（v1.5、CSVマークダウン出力有効時）**
     - `csv_markdown_generated`: 生成されたCSVマークダウンファイル数
     - `csv_sheets_included`: CSVマークダウンに含まれたシート数
     - `csv_errors`: CSVマークダウン出力時のエラー数

### 試験パターン

#### パターン1: 基本試験（v1.3、v1.4）
```bash
python batch_test.py --version [バージョン]
```
- Markdown出力のみを行う基本的な試験

#### パターン2: 基本試験（v1.5、デフォルト）
```bash
python batch_test.py --version 1.5
```
- Markdown出力に加えて、CSVマークダウンファイルも出力（デフォルト動作）
- **CSVマークダウンの形式**：
  - 各シートの印刷領域をCSV形式のコードブロックとして記載
  - シート名を見出しとして記載
  - ファイル末尾に検証用メタデータ（JSON形式）を付記
- ファイル名形式：`{basename}_csv.md`

### CSVマークダウンの詳細仕様
- **出力先**: Markdownファイルと同じディレクトリ（例：`20251110001/実験1/`）
- **ファイル名**: `{basename}_csv.md`（例：`sample_csv.md`）
- **出力対象範囲**: 印刷領域（`csv_output_range="print_area"`、固定）
- **ファイル構成例**:
  ```markdown
  # CSV出力: sample.xlsx

  このファイルは、Excelファイル `sample.xlsx` の各シートの内容をCSV形式で記載しています。
  ...
  **注**: セル内にカンマ、引用符、改行が含まれる場合、CSV形式（RFC 4180）に従って自動的にエスケープされます。上記の例では：
  - カンマを含むセル: 引用符で囲む（`"Value with, comma"`）
  - 引用符を含むセル: 引用符を2つ重ねる（`"Value with ""quotes"""`）
  - 改行を含むセル: 引用符で囲む（`"Value with\nnewline"`）
  ...
  ```json
  {
    "excel": {
      "sheet_count": 2,
      "sheets": [
        {"name": "Sheet1", "rows": 2, "cols": 3},
        {"name": "Sheet2", "rows": 2, "cols": 2}
      ]
    },
    "csv_output": {
      "sheet_count": 2,
  ...
  **検証失敗時の例**（Excelに3シートあるが、CSVマークダウンには2シートのみ出力された場合）:
  ```json
  {
    "excel": {
      "sheet_count": 3,
      "sheets": [
        {"name": "Sheet1", "rows": 10, "cols": 5},
        {"name": "Sheet2", "rows": 8, "cols": 3},
        {"name": "Sheet3", "rows": 5, "cols": 4}
      ]
    },
    "csv_output": {
      "sheet_count": 2,
  ...
  **行数・列数の不一致例**:
  ...
  ```
  ```
- **エラーハンドリング**: CSVマークダウン出力に失敗した場合でも、Markdown出力は継続する

### 実装要件
- Pythonスクリプトとして実装
- **バージョン分岐**：コマンドライン引数で `--version 1.3`、`--version 1.4`、または `--version 1.5` を指定可能（既定値: `1.5`）
- **v1.3の場合**：`v1.3/excel_to_md_v1_3.py` を呼び出す形で実装
- **v1.4の場合**：`v1.4/excel_to_md_v1_4.py` を呼び出す形で実装し、`--mermaid-enabled` オプションを自動的に付与
  - **実験3と実験4のファイル**：`--mermaid-detect-mode shapes` オプションを自動的に付与（シェイプ検出モード）
- **v1.5の場合**：`v1.5/excel_to_md_v1_5.py` を呼び出す形で実装し、`--mermaid-enabled` オプションを自動的に付与
  - **実験3と実験4のファイル**：`--mermaid-detect-mode shapes` オプションを自動的に付与（シェイプ検出モード）
  - **CSVマークダウン出力**：デフォルトで有効（`csv_markdown_enabled=true`）。各シートの印刷領域をCSV形式のコードブロックとして記載したマークダウンファイルを出力
    - CSVマークダウンには検証用メタデータ（JSON形式）が自動的に含まれる
  - **CSVマークダウン検証（--verify オプション指定時）**：
    - 各CSVマークダウンファイルに対して `v1.5/verify_csv_markdown.py` を実行
    - 検証結果が「FAILED」の場合のみ、ユーザーに通知（WARNメッセージ）
    - 検証結果が「OK」の場合は、処理継続（メタデータは既にCSVマークダウンに含まれている）
    - 検証統計（OK数、FAILED数、合格率）を最終サマリーに表示
- コマンドライン引数で実行日付の指定を可能にする（未指定時は実行日を使用）
- 既存のディレクトリが存在する場合は、通番を自動インクリメントして新規ディレクトリを作成

### CSVマークダウン検証用スクリプトの仕様

#### 概要
CSVマークダウンファイル（`{basename}_csv.md`）の内容を検証し、元のExcelファイルと一致するかを確認するスクリプト。
単独で実行可能であり、メインスクリプト（`excel_to_md_v1_5.py`）からも呼び出し可能。

#### スクリプト名
- `v1.5/verify_csv_markdown.py`
...
**コマンドライン引数**:
```bash
python v1.5/verify_csv_markdown.py <excel_file> <csv_markdown_file> [options]
```

**引数**:
- `excel_file`: 元のExcelファイルのパス（必須）
- `csv_markdown_file`: 検証対象のCSVマークダウンファイルのパス（必須）
- `--update-metadata`: 検証結果をCSVマークダウンファイルのメタデータセクションに書き込む（オプション）
- `--verbose`: 詳細な検証結果を標準出力に表示（オプション）
- `--config`: 設定ファイルのパス（オプション、デフォルトは `config.yaml`）

**出力**:
- 標準出力: 検証結果のサマリー（OK/FAILED）
- 終了コード:
  - `0`: 検証成功（OK）
  - `1`: 検証失敗（FAILED）
  - `2`: エラー（ファイルが見つからない、パースエラーなど）
...
def verify_csv_markdown(
    ...
    Args:
        excel_path: 元のExcelファイルのパス
        csv_markdown_path: CSVマークダウンファイルのパス
        config: 設定辞書（オプション）
...
1. **Excelファイルの解析**
   - Excelファイルを読み込み（`openpyxl`を使用）
   - 各シートの印刷領域を取得
   - 印刷領域が未設定の場合は `no_print_area_mode` に従って処理
     - `skip_sheet`: そのシートを検証対象から除外
     - `used_range`: UsedRangeを使用
     - `entire_sheet_range`: シート全体を使用
   - 各シートの行数・列数を計算
   - Excel情報をJSON形式で保持

2. **CSVマークダウンファイルの解析**
   - CSVマークダウンファイルを読み込み
   - 各シートのCSVコードブロックを抽出（`## <シート名>` の見出しと ` ```csv ... ``` ` ブロックをパース）
   - 各CSVブロックの行数・列数をカウント
     - **重要**: Pythonの標準`csv`モジュール（`csv.reader`）を使用してCSVをパース
     - セル内改行や引用符のエスケープを正しく処理（RFC 4180準拠）
     - **空行も含めてすべての行をカウント**（印刷領域が矩形範囲なので、途中の空行も1行として扱う）
     - 例: A1とA5のみにデータがある場合、印刷領域が`A1:A5`なら5行としてカウント（A2, A3, A4は空行だが含まれる）
     - 各行の列数をカウントし、最大列数を記録
   - CSV出力情報をJSON形式で保持

3. **検証の実行**
   - **シート数の検証**: `excel.sheet_count == csv_output.sheet_count`
   - **各シートの存在確認**: Excelの全シートがCSV出力にも存在するか
   - **各シートの行数・列数の検証**:
     - `excel.sheets[i].rows == csv_output.sheets[i].rows`
     - `excel.sheets[i].cols == csv_output.sheets[i].cols`
   - すべての検証が成功した場合 → `status: "OK"`
   - いずれかの検証が失敗した場合 → `status: "FAILED"`

4. **結果の出力**
   - 検証結果を辞書形式で返す
   - `--update-metadata` が指定された場合、CSVマークダウンファイルの末尾にメタデータセクションを追記または更新
   - `--verbose` が指定された場合、詳細な検証結果を標準出力に表示

#### メタデータの更新処理

**メタデータセクションの検出**:
- CSVマークダウンファイルを末尾から読み込み
- `## 検証用メタデータ` の見出しを検索
- 既存のメタデータセクションがある場合は削除
...
- **Excel読み込みエラー**: ファイルが存在しない、openpyxlで開けない → 終了コード2、エラーメッセージを出力
- **CSVマークダウンパースエラー**: ファイルが存在しない、形式が不正 → 終了コード2、エラーメッセージを出力
- **設定ファイル読み込みエラー**: 設定ファイルが不正 → WARN、デフォルト設定を使用
- **メタデータ書き込みエラー**: ファイルへの書き込み権限がない → WARN、検証結果は返す

#### 依存ライブラリ

- `openpyxl`: Excelファイルの読み込み
- `pyyaml`: 設定ファイルの読み込み
- 標準ライブラリ: `argparse`, `json`, `csv`, `re`, `pathlib`

#### メインスクリプトからの呼び出し

`excel_to_md_v1_5.py` の処理フロー:
1. CSVマークダウンファイルを出力（メタデータなし）
2. `csv_include_metadata=true` の場合:
   - `verify_csv_markdown()` を呼び出して検証を実行
   - 検証結果をCSVマークダウンファイルの末尾に追記

#### テスト要件

- 単体テスト: 各関数の動作確認
- 統合テスト: ExcelファイルとCSVマークダウンの検証
- エッジケース:
  - 印刷領域未設定のシート
  - 空のシート
  - シート数の不一致
...
# 付録A.x+1：実装詳細ガイド — ディスパッチ／列名マッチング／フォールバック（新設）

## A.x+1.1 ディスパッチの導入方法（フォールバック手順を明確化・修正）
...
## A.x+1.2.5 シェイプ（図形）からのMermaid抽出（新設）
...
## A.x+1.5 `header_detection=none` と元テーブル出力
